...
def predict(width, height, confidences, boxes,
            prob_threshold, iou_threshold=0.5, top_k=-1):
    """
...
def detect_faces(ort_session, img_origin):
    h, w = img_origin.shape[:2]
    img = cv2.resize(img_origin, (640, 480))
...
staff_img_paths = glob.glob(os.path.join('staffs/*'))
staff_imgs = [cv2.imread(img_path) for img_path in staff_img_paths]
names_list = [
    os.path.splitext(os.path.basename(img_path))[0]
    for img_path in staff_img_paths
]
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        for img in staff_imgs:
            faces, boxes = detect_faces(ort_session, img)
            if len(faces) == 0:
                raise Exception('Image contains no face')
        ...
        feed_dict = {
            images_placeholder: face_imgs,
            phase_train_placeholder: False,
        }
        saved_embeds = sess.run(embeddings, feed_dict=feed_dict)
        ...
        while True:
            ...
            faces, boxes = detect_faces(ort_session, frame)
            ...
            if len(faces) > 0:
                ...
                for i in range(boxes.shape[0]):
                    ...
                    cv2.putText(
                        frame, text,
                        (x1 + 6, y2 - 6), font, 0.3, (255, 255, 255), 1)
